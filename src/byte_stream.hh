...
public:
  ...
  uint64_t get_capacity() const { return capacity_; }; // Get the capacity of the stream
...
protected:
  ...
  // Reader.peek() should return a string_view which must hold contiguous bytes
  // for buffer data. So the ring buffer or queue can not be used here. Here we
  // use lazily compact (erase the consumed prefix) only when the head index
  // grows large enough)
  std::string buffer_ = "";
  size_t head_ = 0; // The head of buffer.
